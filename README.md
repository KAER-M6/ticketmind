...
> 文档导航：[设计说明.md](设计说明.md)（搭建思路 · 架构 · 功能说明）· [部署说明.md](部署说明.md)（部署步骤 · 数据资产）

## 功能说明

| 功能 | 说明 |
|---|---|
| 单条工单智能处理 | 粘贴工单 → 数秒内输出类型、优先级、判定理由、置信度、相似案例、候选回复 |
| 批量导入 | 多条粘贴或上传文本，可选是否走 AI 全链路；AI 模式线程池并发（默认 6） |
| 混合检索相似案例 | 从 19,782 条历史工单召回 top-5，双路召回 + RRF 融合，hit@5 = 99.5% |
| 回复草稿生成 | 基于检索案例生成候选回复并标注引用来源，平均引用 4 条 |
| 人工审核状态机 | 通过 / 驳回 / 完成三态流转，**通过时可下拉纠正 type 与 priority** |
| 反馈闭环 | 纠正标签自动回流语料池参与后续 few-shot，界面显示线上准确率 |
| 中文工单支持 | 中文走语言桥接层检索，回复默认输出中文（`TICKETMIND_REPLY_LANG` 可强制 zh/en） |
| KPI 看板 | 工单总量、类型分布、自动化覆盖率、线上准确率、反馈池规模、检索缓存命中率 |
| REST API | 10 个端点，可脱离界面对接外部系统 |
| 安全与容错 | `X-API-Key` 鉴权中间件、CORS 白名单、超时重试、降级回退 |

各功能的设计取舍与实现细节见 **[设计说明.md](设计说明.md)**。
...
| 文档 | 内容 |
|---|---|
| [`设计说明.md`](设计说明.md) | 为什么这么搭：架构图、一次工单的完整旅程、技术选型取舍表、被证伪的方案、迭代过程 |
| [`部署说明.md`](部署说明.md) | 怎么跑起来：环境要求、部署步骤、数据重建、目录说明 |
| `eval/` | 评估报告原始 JSON（分诊 / 检索 / 回复 / 反馈增益 / golden 回归） |
| `scripts/demo_pipeline.py` | 命令行跑单条工单的完整链路 |
...
详见 [设计说明.md](设计说明.md) 与 [部署说明.md](部署说明.md)。
